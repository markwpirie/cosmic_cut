...
| **9** | Swap in Pixi.js, real sprites, glass blocks, neon, particles, juice. | Graphics layer, polish | ✅ Shipped to `main` — bloom, rounded glass, energy enemies, Orbitron HUD, boss stages; full art-direction pass complete. **Canvas renderer retired 2026-07-13**: `render.js` deleted, Pixi is now the only renderer (no more `?pixi` flag). **Zone palette recoloured 2026-07-07** (below) — the cyan-hero flattening read too subtle; superseded by a clear per-zone hue. See PHASE9.md. Boss picture-reveal ✅ built 2026-07-07 (§7). Sprite-based stars/particles + glass-block depth still open. |
...
- **Phase 9 graphics: shipped, and Pixi is now the only renderer.** The presentation layer runs on **Pixi.js v8** (loaded as a CDN ES module via an importmap — *no build step*, keeping the serve-the-folder model and avoiding a Node/npm dependency). It launched **opt-in via `?pixi`** alongside the old canvas renderer (`render.js`) while it matured; once it covered every visual case, the canvas renderer was retired (2026-07-13) and `render-pixi.js` now loads unconditionally. It reuses every logic module untouched (design principle §1.2). See PHASE9.md for the build history. Phase 9 was done **before** Phase 7 (touch) per the build order ✓
...
- **Audio + Feel pass 2 (drop-in soundtrack + director + beat visuals):** pulls more of the Phase 9/10 buckets forward. All feel knobs centralised in `config.AUDIO`.
  ...
  - **Pause (extended 2026-07-13):** **P / Esc** freezes play (also intro/death), dims the board, and ducks music + movement/cut tones. While paused it's now an interactive menu — **RESUME / SFX / MUSIC / QUIT TO MENU** — keyboard nav (↑↓ select, ←→ adjust, Enter/Space activate) or touch (a dedicated on-screen pause button, since phones have no Esc key, plus tap-to-hit-test on the rows themselves). SFX and music each have an independent, persisted 0-100% volume slider on top of the existing M-mute/N-music toggles. Quitting commits the run's score to the high-score table before returning to stage-select, so bailing early doesn't forfeit a high score.
