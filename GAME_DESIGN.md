...
- **Game-Feel pass (procedural, no assets):** all in `audio.js` (Web-Audio) + `fx.js` (pure-maths particles/shake), wired from `main`:
  - **Audio:** retro SFX (claim/kill/death/UI/level-clear/game-over/high-score), a **rising cut-tension tone** while cutting (pitch tracks cut length), the bonus **"doof doof doof"** stingers timed to the read-out, and a **generative minor-pentatonic synthwave loop** whose tempo/brightness swell with danger. **M** mutes, **N** toggles music; both persist. Context resumes on first keypress.
